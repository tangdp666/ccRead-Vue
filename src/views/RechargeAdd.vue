<template>
    ...
    <div class="rechargeAddMain">
        <span style="font-size: 20px;margin-left: 5%;">我的充值</span>
        <div>
            <n-list style="padding-left: 5%;padding-right: 5%;padding-top: 10px;">
                <n-list-item v-for="(item, index) in rechargeInfo" :key="index">
                    <span style="font-size: 17px;font-weight: bold">+{{ item.money }}</span>
                    <span style="float: right;font-size: 14px;color: #7d7d7d;">{{ item.createTime }}</span>
                </n-list-item>
            </n-list>
        </div>
    </div>
...
const getRechargInfo = async () =>{
    let res = await axios.get("/ccread-order/order/getRechargeList/1")
    console.log(res)
